...
<header style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
    <div>
        ...
        <p style="margin: 4px 0 0 0; color: #64748b; font-size: 0.85rem;">Kelola data anggota peminjam dan lakukan transaksi peminjaman buku.</p>
    </div>
...
<section class="panel" id="loanFormSection" style="display: none; margin-bottom: 24px;">
    <div class="panel-heading" style="border-bottom: 2px solid #00a65a; padding-bottom: 12px; margin-bottom: 16px;">
        <h2 style="font-size: 1.1rem; color: #0f172a; margin: 0; display: flex; align-items: center; gap: 8px;">
            <span>📖</span> Form Transaksi Peminjaman Buku
        </h2>
    </div>

    <form id="onsiteLoanForm" onsubmit="processOnsiteLoan(event)">
        <!-- 1. DATA SISWA / PEMINJAM -->
        <div style="margin-bottom: 20px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                <input type="text" id="user_code_input" placeholder="Scan kartu siswa / ketik nomor telepon / nama..." required style="flex: 1; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 4px; font-size: 0.88rem; font-weight: 600;">
                ...
                <tr>
                    ...
                    <td style="text-align: center;">
                        <div style="display: flex; gap: 6px; justify-content: center;">
                            <button type="button" onclick="selectUserForLoan('{{ $user->telepon }}')" class="btn btn-success" style="padding: 4px 8px; font-size: 0.75rem;" title="Pilih anggota ini untuk peminjaman">⚡ Pinjamkan</button>
...
function toggleLoanForm() {
    ...
    if (formSection.style.display === 'none' || !formSection.style.display) {
        ...
        document.getElementById('user_code_input').focus();
    } else {
        formSection.style.display = 'none';
    }
}

function selectUserForLoan(userPhone) {
    const formSection = document.getElementById('loanFormSection');
    formSection.style.display = 'block';
    userCodeInput.value = userPhone;
    lookupUserByCode(userPhone);
    formSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
